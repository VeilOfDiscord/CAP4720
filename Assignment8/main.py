...
translation_mat_L = pyrr.matrix44.create_from_translation(-obj.center)

scaling_mat = pyrr.matrix44.create_from_scale([2 / obj.dia, 2 / obj.dia, 2 / obj.dia])

model_mat = pyrr.matrix44.multiply(translation_mat_L, scaling_mat)
...
shading_type_radio_button = gui.add_radio_buttons("shading type", options_dict={"diffuse": 1,
                                                                                         "toon": 4},
                                                                        initial_option="diffuse")

silhouette_checkbox = gui.add_checkbox("Silhouette", initial_state=True)
...
while draw:
    ...
    ID_str = shading_type_radio_button.get_value()
    ID_value = 0
    if ID_str == "1":
        ID_value = 1.0
    if ID_str == "4":
    ...
    shaderProgram["sil"] = silhouette_checkbox.get_value()
